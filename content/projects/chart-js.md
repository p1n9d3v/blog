...
하지만 색을 지정할 수는 없었는데 따로 chart.js에서 제공해주지는 않아 직접 만들었다.
...
업데이트 되기 전에 라벨의 줄의 개수를 계산하여 라벨의 공간을 확보하였다.
...
라벨에 색을 지정하는 것은 어렵지 않았으나 문제가 되었던 부분이 라벨이 겹치지 않도록 회전을 시켜줘야 했다. 라벨의 픽셀 길이와 틱의 차이를 구하여 회전 각도를 동적으로 조정하였다.
